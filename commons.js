...
async function updateShopifyInventory(updates, locationId = process.env.DEFAULT_LOCATION_ID) {
  ...
  if (!locationId) {
    throw new Error("❌ Manca locationId. Passalo come parametro o inseriscilo in SHOPIFY_LOCATION_ID");
  }
  ...
  try {
    ...
    if (userErrors.length > 0) {
      userErrors.forEach((err, i) => {
        ...
        console.error(`❌ [${i + 1}] Errore item: ${errorEntry.inventoryItemId}`);
