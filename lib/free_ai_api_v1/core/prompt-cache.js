 ...
 * Thread-safe per l'event loop di Node.js (operazioni sincrone su Map).
 */
const fs = require("fs");
...
class PromptCache {
  constructor(options = {}) {
    this.cache = new Map();
    this.ttlMs = options.ttlMs || 7 * 24 * 60 * 60 * 1000; // 7 giorni
  ...
  load() {
    try {
      ...
      // Cache corrotta o illeggibile: ignora silenziosamente
    }
  }
